...
public class NonPreemptivePriority {
    ...
    private void executeNonPriority() {
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            }); // Sort procedures by process name
            ...
            while (!pd.isEmpty() || !readyQueue.isEmpty()) {
                while (!pd.isEmpty() && pd.get(0).arrivalTime <= currentTime) {
                ...
                if (!readyQueue.isEmpty()) {
                    ...
                    int finishTime = currentTime + currentProcedure.burstTime;
                    int turnaroundTime = finishTime - currentProcedure.arrivalTime;
                    int waitingTime = turnaroundTime - currentProcedure.burstTime;

                    totalTurnaroundTime += turnaroundTime;
                    totalWaitingTime += waitingTime;

                    String[] rowData = {
                            currentProcedure.processName, 
                            String.valueOf(currentProcedure.arrivalTime),
                            String.valueOf(currentProcedure.burstTime),
                            String.valueOf(finishTime),
                            String.valueOf(currentProcedure.priority),
                    ...
                    upperLine.append("-".repeat(Math.max(0, currentProcedure.burstTime))).append("-----");
                    ganttChart.append(String.format(" %-" + ganttChartWidth + "s |", currentProcedure.processName));
                    lowerLine.append("-".repeat(Math.max(0, currentProcedure.burstTime))).append("-----");
            ...
            System.out.printf("| %-8s | %-16s | %-16s | %-16s | %-16s | %-16s | %-16s |\n", "Process", "Arrival Time",
                    "Burst Time", "Finish Time", "Priority", "Turnaround Time", "Waiting Time");
            System.out.println(
                    "+----------+------------------+------------------+------------------+------------------+------------------+------------------+");
